...
{
    class ThanhToan_TraHang
    {
        public void ThanhToan_TH(XuLyMayInPos xuly_, string tennt, string dienthoai, string sott, string maytn, string nvban, string nhap, DataTable banhangct, decimal thu, bool inCT, bool inlai)
        {
            ...
            if (tag != null)
            {
                try
                {
                    if (inlai == true)
                    {
                        PrintReceiptTwoHeader(xuly_, "HOA DON TRA HANG", "( IN LAI )", sott, maytn, xuly_.ConvertToUnSign(nvban), nhap, "MA SP", "TEN SP");
                    }
                    else
                    {
                        PrintReceiptHeader(xuly_, "HOA DON TRA HANG", sott, maytn, xuly_.ConvertToUnSign(nvban), nhap, "MA SP", "TEN SP");
                    }
                    ...
                    for (int i = 0; i < banhangct.Rows.Count; i++)
                    {
                        ...
                        else
                        {
                            if (tttra_ != 0 && tttra_ > 0)
                            {
                                InChiTietCoGiamGiaKoLoDate(xuly_, masp_, tensp_, SLTra_, tendvt_, "X", DonGiaTracoTAX, "=", TTTraCoTAX, mucgiam_, TTgiamgia_, TTthucte_);
                            }
                            else
                            {
                                InChiTietKoGiamGiaKoLoDate(xuly_, masp_, tensp_, SLTra_, tendvt_, "X", DonGiaTracoTAX, "=", TTTraCoTAX);
                            }
                        }
                    }
                    PrintReceiptFooter(xuly_, thu, DateTime.Now, "Cam on quy khach!", "ECO PHARMACY", xuly_.ConvertToUnSign(tennt), dienthoai);
                    xuly_.Disable(tag);
                }
                catch {
                    xuly_.Disable(tag);
        ...
        public void PrintReceiptFooter(XuLyMayInPos xuly_, decimal thu_, DateTime dateTime, string footerText, string companyName, string tent_, string tell)
        {
            ...
            //xuly_.InChinhSua( offSetString + new string('-', (printer.RecLineChars)));
        ...
        public void InChiTietCoGiamGia(XuLyMayInPos xuly_,  string itemCode, string Description, string lot, string date, decimal @slban, string dvt, string nhan, decimal @DonGiaBancoTAX, string bang, decimal @TTbancoTAX, decimal @mucgiam_, decimal @ttgiam_, decimal @TTthucte_)
        {
            ...
            xuly_.InBinhThuong( xuly_.CatBoBot(bang.PadLeft(4), 4));
            xuly_.InChinhSua( xuly_.InCanhPhai(@TTbancoTAX.ToString("#,###")));
            xuly_.InBinhThuong(String.Format("PHI  {0}%", @mucgiam_.ToString("#,###.##")));
            xuly_.InBinhThuong( String.Format("    ({0})", @ttgiam_.ToString("#,###")));
            xuly_.InChinhSua( xuly_.InCanhPhai(@TTthucte_.ToString("#,###")));
        }
        public void InChiTietKoGiamGiaKoLoDate(XuLyMayInPos xuly_,  string itemCode, string Description, decimal @slban, string dvt, string nhan, decimal @DonGiaBancoTAX, string bang, decimal @TTbancoTAX)
        {
            xuly_.InBinhThuong( xuly_.CatBoBot(itemCode.PadRight(6), 6));
            xuly_.InChinhSua( xuly_.CatBoBot(Description.PadLeft(9), 27));
            //xuly_.InBinhThuong( String.Format("Lô: {0}", lot.PadLeft(3), 3));
        ...
        public void InChiTietCoGiamGiaKoLoDate(XuLyMayInPos xuly_,  string itemCode, string Description, decimal @slban, string dvt, string nhan, decimal @DonGiaBancoTAX, string bang, decimal @TTbancoTAX, decimal @mucgiam_, decimal @ttgiam_, decimal @TTthucte_)
        {
            xuly_.InBinhThuong( xuly_.CatBoBot(itemCode.PadRight(6), 6));
            xuly_.InChinhSua( xuly_.CatBoBot(Description.PadLeft(9), 27));
            //xuly_.InBinhThuong( String.Format("Lô: {0}", lot.PadLeft(3), 3));
