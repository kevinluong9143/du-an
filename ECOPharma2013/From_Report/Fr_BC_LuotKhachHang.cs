...
{
    public partial class Fr_BC_LuotKhachHang : Form
    {
        public Fr_BC_LuotKhachHang()
        {
            InitializeComponent();
        }
        ...
        DataTable dtbLuotKhachHang = new DataTable();
        private void btnXem_Click(object sender, EventArgs e)
        {
            ...
            CSQLKhachHang khachang = new CSQLKhachHang();

            if (cboNhaThuoc.SelectedValue != null && cboNhaThuoc.SelectedIndex != -1)
            {
                dtbLuotKhachHang = khachang.BaoCaoLuotKhachHang_LoadDSTuNgay_DenNgay_NTid(TuNgay, DenNgay, cboNhaThuoc.SelectedValue.ToString());

                if (dtbLuotKhachHang != null && dtbLuotKhachHang.Rows.Count > 0)
                {
                    rgvLuotKhachHang.DataSource = dtbLuotKhachHang;
                }
                else
                {
                ...
                MessageBox.Show("Vui lòng chọn nhà thuốc để kiểm tra!");
        ...
        {
            if (rgvLuotKhachHang.Rows.Count > 0)
            {
                ...
                if (f.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (qtc.KiemTraQuyenIn("Fr_BC_LuotKhachHang") == false)
                    {
                        MessageBox.Show("Bạn không có quyền truy cập chức năng này!");
                        return;
                    }
                    ExportToExcelML exporter = new ExportToExcelML(this.rgvLuotKhachHang);
        ...
        private void Fr_BC_LuotKhachHang_Load(object sender, EventArgs e)
        {
