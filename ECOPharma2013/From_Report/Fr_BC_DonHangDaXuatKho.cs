...
{
    public partial class Fr_BC_DonHangDaXuatKho : Form
    {
        public Fr_BC_DonHangDaXuatKho()
        {
            InitializeComponent();
        }
        ...
        private void btnXem_Click(object sender, EventArgs e)
        {
            ...
            dtbNhanTraHang = px_.BaoCaoDonHangDaXuatKho_LoadDSTuNgay_DenNgay(TuNgay, DenNgay);
            if (dtbNhanTraHang != null && dtbNhanTraHang.Rows.Count > 0)
            {
                rgvDSPhieuXK.DataSource = dtbNhanTraHang;
            }
            else
            {
        ...
        private void btnExportToExcel_Click(object sender, EventArgs e)
        {
            if (rgvDSPhieuXK.Rows.Count > 0)
            {
                ...
                if (f.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (qtc.KiemTraQuyenIn("Fr_BC_DonHangDaXuatKho") == false)
                    {
                        MessageBox.Show("Bạn không có quyền truy cập chức năng này!");
                        return;
                    }
                    Fr_DangXuLy.ShowFormCho();
                    ExportToExcelML exporter = new ExportToExcelML(this.rgvDSPhieuXK);
                    exporter.ExportVisualSettings = true;

                    string tempPath = Path.GetTempPath();
                    tempPath += @"\DonHangDaXuatKho.xls";
        ...
        private void Fr_BC_DonHangDaXuatKho_Load(object sender, EventArgs e)
        {
